...
class User extends CI_Controller
{
    ...
    public function index()
    {
        ...
        $this->session->set_flashdata('not-login', false);
        $this->session->set_flashdata('fail-pass', false);
        $this->session->set_flashdata('fail-email', false);
        $this->session->set_flashdata('fail-login', false);
        $this->session->set_flashdata('success-reg', false);
        $this->session->set_flashdata('login-success', false);
        $this->session->set_flashdata('success-verify', false);
        $this->session->set_flashdata('success-logout', false);
        $this->session->set_flashdata('false-login', false);
    ...
    public function tingkatsd()
    {
        $data['user'] = $this->db->get_where('siswa', ['email' =>
            $this->session->userdata('email')])->row_array();

        $this->load->view('user/tingkatsd');
        $this->load->view('template/footer');
    }

    public function tingkatsmp()
    {
        $data['user'] = $this->db->get_where('siswa', ['email' =>
            $this->session->userdata('email')])->row_array();

        $this->load->view('user/tingkatsmp');
        $this->load->view('template/footer');
    }

    public function tingkatsma()
    {
        $data['user'] = $this->db->get_where('siswa', ['email' =>
            $this->session->userdata('email')])->row_array();

        $this->load->view('user/tingkatsma');
        $this->load->view('template/footer');
    }
